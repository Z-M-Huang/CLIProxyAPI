...
func TestAntigravityUserAgentDefaultFromConfig(t *testing.T) {
	...
	if got := resolveLoadCodeAssistUserAgent(cfg, nil); got != "antigravity/1.23.2 linux/x64" {
		t.Fatalf("resolveLoadCodeAssistUserAgent configured = %q, want antigravity/1.23.2 linux/x64", got)
	}
}
