...
func (e *AntigravityExecutor) buildRequest(ctx context.Context, auth *cliproxyauth.Auth, token, modelName string, payload []byte, stream bool, alt, baseURL string) (*http.Request, error) {
	...
	bodyReader, payloadLog := (&antigravityPayloadBuilder{
		payload:    payload,
		modelName:  modelName,
...
func (b *antigravityPayloadBuilder) build() (io.Reader, []byte) {
	...
	if antigravityRequestNeedsSchemaSanitization(payload) {
		...
		if b.requestLog {
			return strings.NewReader(payloadStr), []byte(payloadStr)
		}
		return strings.NewReader(payloadStr), nil
	}
	...
	if b.requestLog {
		return bytes.NewReader(payload), append([]byte(nil), payload...)
	}
