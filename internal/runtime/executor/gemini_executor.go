...
func (e *GeminiExecutor) PrepareRequest(req *http.Request, auth *cliproxyauth.Auth) error {
	...
	applyGeminiHeaders(req, auth, e.cfg)
	return nil
}
...
func (e *GeminiExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	applyGeminiHeaders(httpReq, auth, e.cfg)
...
func applyGeminiHeaders(req *http.Request, auth *cliproxyauth.Auth, cfg *config.Config) {
	if userAgent := geminiUserAgent(cfg); userAgent != "" {
...
func geminiUserAgent(cfg *config.Config) string {
	if cfg == nil {
		return ""
	}
	if userAgent := strings.TrimSpace(cfg.GeminiHeaderDefaults.UserAgent); userAgent != "" {
		return userAgent
	}
	return strings.TrimSpace(cfg.GeminiCLIHeaderDefaults.UserAgent)
}
