...
func TestGeminiUserAgentFromConfig(t *testing.T) {
	if got := geminiUserAgent(&config.Config{}); got != "" {
		t.Fatalf("geminiUserAgent default = %q, want empty", got)
	}

	legacy := &config.Config{
		GeminiCLIHeaderDefaults: config.GeminiCLIHeaderDefaults{UserAgent: "legacy-gemini/1.0"},
	}
	if got := geminiUserAgent(legacy); got != "legacy-gemini/1.0" {
		t.Fatalf("geminiUserAgent legacy = %q, want legacy-gemini/1.0", got)
	}

	cfg := &config.Config{
		GeminiHeaderDefaults:    config.GeminiHeaderDefaults{UserAgent: "gemini-api/2.0"},
		GeminiCLIHeaderDefaults: config.GeminiCLIHeaderDefaults{UserAgent: "legacy-gemini/1.0"},
	}
	req := httptest.NewRequest(http.MethodPost, "https://example.com", nil)
	applyGeminiHeaders(req, &cliproxyauth.Auth{Attributes: map[string]string{
		"header:User-Agent": "credential-override/3.0",
	}}, cfg)
	if got := req.Header.Get("User-Agent"); got != "credential-override/3.0" {
		t.Fatalf("User-Agent = %q, want credential-override/3.0", got)
	}
}
