...
// WithUsageStore wires persistent usage statistics and request history into management APIs.
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if optionState.usageStore != nil {
		optionState.usageStore.SetUsageEventRetentionDays(cfg.UsageEventRetentionDays)
	}
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		// FORK[usage-sqlite]: stable persisted usage and request-history endpoints.
		...
		// FORK[prompt-rules]: management surface for fork-owned prompt rules.
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	if s.usageStore != nil && (oldCfg == nil || oldCfg.UsageEventRetentionDays != cfg.UsageEventRetentionDays) {
		s.usageStore.SetUsageEventRetentionDays(cfg.UsageEventRetentionDays)
	}
