...
// FORK[refresh-backoff]: repeated transient refresh failures back off exponentially.
func nextRefreshFailureBackoff(failures int) time.Duration {
	...
	for i := 1; i < failures; i++ {
		if backoff >= refreshFailureBackoffMax/2 {
			return refreshFailureBackoffMax
		}
		backoff *= 2
	}
	if backoff > refreshFailureBackoffMax {
		return refreshFailureBackoffMax
	}
	return backoff
}
		...
		if current := m.auths[id]; current != nil {
			...
			if unauthorized {
				...
				if m.refreshFailures == nil {
					m.refreshFailures = make(map[string]int)
				}
				failures := m.refreshFailures[id] + 1
				m.refreshFailures[id] = failures
				current.NextRefreshAfter = now.Add(nextRefreshFailureBackoff(failures))
			}
			m.auths[id] = current
